...
public class DriveStraightFeet extends CommandBase {
	...
	protected void execute() {
		...
		// We take the absolute values, since we just need the total distance traveled if the robot is going backwards
				...
				distanceSoFar = Math.sqrt((currentN - startN) * (currentN - startN) + (currentE - startE) * (currentE - startE));
				double feetRemaining = feet - distanceSoFar;
				// This ramps the power down to zero when we reach the target distance
				double powerMod = MathHelper.clamp(power * feetRemaining / SLOWING_DOWN_DISTANCE,
						-Math.abs(power), Math.abs(power));
